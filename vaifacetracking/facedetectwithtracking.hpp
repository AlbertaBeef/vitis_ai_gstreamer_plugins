...
FaceDetectWithTracking::run(const cv::Mat &input_image) {
  cv::Mat image;
  auto size = cv::Size(face_detect_->getInputWidth(), face_detect_->getInputHeight());
  ...
  for (const auto &r : face_results.rects) {
    int x1 = r.x * input_image.cols;
    int y1 = r.y * input_image.rows;
    int x2 = x1 + (r.width * input_image.cols);
    int y2 = y1 + (r.height * input_image.rows);
